...
func commandMigrationStatus() *cobra.Command {
	c := &cobra.Command{
...
func commandMigrationCreate(cfg *config.Migration) *cobra.Command {
	...
	c := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if len(table) > 0 {
				_, err := f1.WriteString(fmt.Sprintf("ALTER TABLE %s\nALTER COLUMN %s TYPE %s;", table, "column_name", "column_definition"))
				if err != nil {
					panic(err)
				}
				_, err = f2.WriteString(fmt.Sprintf("ALTER TABLE %s\nALTER COLUMN %s TYPE %s;", table, "column_name", "column_definition"))
				if err != nil {
					panic(err)
				}
			} else {
				_, err := f1.WriteString(fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s \n(\n);", create))
				if err != nil {
					panic(err)
				}
				_, err = f2.WriteString(fmt.Sprintf("DROP TABLE IF EXISTS %s;", create))
				if err != nil {
					panic(err)
				}
			}
...
func commandMigrationDown() *cobra.Command {
	var input int
	c := &cobra.Command{
		Use:   "migration:down",
